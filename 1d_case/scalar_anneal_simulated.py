...
# --------------------------------------------------
# Choose from T = p: 2, 100, 1000
# --------------------------------------------------
T = 1000   # total evolution time, Large T → slow evolution → closer to adiabatic behavior
p = 1000    # number of discrete time steps (layers), The number of alternating cost/mixer layers, The circuit depth multiplier
dt = T / p
...
filename = f"samples_simu_{tag}.json"
...
plt.tight_layout()
plt.savefig(f"histogram_simu_T{T}_p{p}_eps{eps}.png", dpi=300)
plt.close()
